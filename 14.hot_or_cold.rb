...
# Returns: Nothing
# Behavior: Prompts the user to guess the number "num_to_guess", saying "hot" or "cold" depending on whether the user's guess is too high or too low. Once the user has guessed correctly, the method prints a victory messagße and returns.
...
def get_user_guess
  ...
  def hot_or_cold(num_to_guess)
    ...
    while true
      ...
      if guess < num_to_guess    # The guess is too cold
        ...
        # This "return" will make the program return from hot_or_cold, even from inside the while loop.
        return
      end
    end
  end
  ...
  if __FILE__ == $PROGRAM_NAME
    # rand(1000) returns a random integer between 0 and 999, so this method call starts the "Hot or Cold" game with a random number to guess.
